...
#
class MediaWiktory::Wikipedia::Parse < GetAction
  ...
  # @return [self]
  def pst(value = true)
    merge(pst: 'true') if value
  end
  # Do a pre-save transform (PST) on the input, but don't parse it. Returns the same wikitext, after a PST has been applied. Only valid when used with text.
  #
  # @param value [true, false]
  # @return [self]
  def onlypst(value = true)
    merge(onlypst: 'true') if value
  end
  # Includes language links supplied by extensions (for use with prop=langlinks).
  #
  # @param value [true, false]
  # @return [self]
  def effectivelanglinks(value = true)
    merge(effectivelanglinks: 'true') if value
  end
...
Unlike page editing, this does not fall back to summary when omitted or empty.
  ...
  # @param value [true, false]
  # @return [self]
  def disablepp(value = true)
    merge(disablepp: 'true') if value
  end
  # Omit the limit report ("NewPP limit report") from the parser output.
  #
  # @param value [true, false]
  # @return [self]
  def disablelimitreport(value = true)
    merge(disablelimitreport: 'true') if value
  end
  # Omit edit section links from the parser output.
  #
  # @param value [true, false]
  # @return [self]
  def disableeditsection(value = true)
    merge(disableeditsection: 'true') if value
  end
  # Do not run HTML cleanup (e.g. tidy) on the parser output.
  #
  # @param value [true, false]
  # @return [self]
  def disabletidy(value = true)
    merge(disabletidy: 'true') if value
  end
  ...
  # @param value [true, false]
  # @return [self]
  def preview(value = true)
    merge(preview: 'true') if value
  end
  # Parse in section preview mode (enables preview mode too).
  #
  # @param value [true, false]
  # @return [self]
  def sectionpreview(value = true)
    merge(sectionpreview: 'true') if value
  end
  # Omit table of contents in output.
  #
  # @param value [true, false]
  # @return [self]
  def disabletoc(value = true)
    merge(disabletoc: 'true') if value
  end
  ...
  # @param value [true, false]
  # @return [self]
  def mobileformat(value = true)
    merge(mobileformat: 'true') if value
  end
  # Disable images in mobile output.
  #
  # @param value [true, false]
  # @return [self]
  def noimages(value = true)
    merge(noimages: 'true') if value
  end
  # Apply mobile main page transformations.
  #
  # @param value [true, false]
  # @return [self]
  def mainpage(value = true)
    merge(mainpage: 'true') if value
  end
