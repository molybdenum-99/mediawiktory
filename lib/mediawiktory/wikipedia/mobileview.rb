...
#
class MediaWiktory::Wikipedia::Mobileview < GetAction
  ...
  # @param value [true, false]
  # @return [self]
  def noheadings(value = true)
    merge(noheadings: 'true') if value
  end
  # Don't transform HTML into mobile-specific version.
  #
  # @param value [true, false]
  # @return [self]
  def notransform(value = true)
    merge(notransform: 'true') if value
  end
  # Return only requested sections even with prop=sections.
  #
  # @param value [true, false]
  # @return [self]
  def onlyrequestedsections(value = true)
    merge(onlyrequestedsections: 'true') if value
  end
