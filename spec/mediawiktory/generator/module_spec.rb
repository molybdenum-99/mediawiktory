...
RSpec.describe MediaWiktory::Generator::Module do
  ...
  describe '#to_html' do
    ...
    it 'renders class' do
      is_expected.to include('class Dummy::Abusefiltercheckmatch < GetAction')
    end

    it 'renders generic docs' do
      is_expected.to include("# Check to see if an AbuseFilter matches a set of variables, editor logged AbuseFilter event.\n"\
        "#\n"\
        "# vars, rcid or logid is required however only one may be used."
